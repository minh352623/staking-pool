...
describe("nft-staking", () => {
  ...
  // "https://blue-black-patina.solana-testnet.quiknode.pro/db122dca8d6ae35e8f5769dd01d010bb398655be/"
  ...
  const programId = new PublicKey(
    "GGWeSTDbz2WCERf59nTxwSRTFLMg7butvdk9tv4D53sN"
  );
  ...
  const log = async (signature: string): Promise<string> => {
    console.log(
      `Your transaction signature: https://explorer.solana.com/transaction/${signature}?cluster=testnet`
    );
    return signature;
  };
  ...
  it("Airdrop", async () => {
    await connection
      .requestAirdrop(keypair.publicKey, LAMPORTS_PER_SOL * 1)
      .then(confirm)
      .then(log);
  });
  ...
  it("Create Collection", async () => {
    ...
    // ];
    // const master_edition_seeds = [...metadata_seeds, Buffer.from("edition")];
    ...
    await mintTo(
      ...
      userWallet.publicKey,
      1000000 * 10 ** 9 // because decimals for the mint are set to 9
    );
  ...
  it("Create Staking Rule", async () => {
    ...
    const decimals = 9;
    stakingRuleAddress = PublicKey.findProgramAddressSync(
      [Buffer.from("rules"), collectionMint.toBuffer()],
      programId
    )[0];

    const signature = await program.methods
      .createStakingRule(decimals, rewardPerUnix)
      .accounts({
  ...
  it("Create Staking Account", async () => {
    stakingAccountAddress = PublicKey.findProgramAddressSync(
      [
        Buffer.from("account"),
        stakingRuleAddress.toBuffer(),
  ...
  it("Verify Collection", async () => {
    ...
    let verifytx = verifyCollection(umi, {
  ...
  it("Stake", async () => {
    stakingInstanceAddress = PublicKey.findProgramAddressSync(
      [
